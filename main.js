...
;~function(global) {
	...
	function generateFloor() {
		...
		while (deepFloors.length>0) {
			...
			var floorX = Math.floor(f["rect"][0] / STAGE_WIDTH * (STAGE_WIDTH - FLOOR_WIDTH * 0.5));
	...
	function removeOutboundFloor() {
		...
		if (floorIndex > 0) {
			//console.log("removeOutboundFloor", floorArray[floorIndex]);
	...
	function updateHeroHorizontalPostion(step, time) {
		...
		if (velocity != 0) {
			...
			if (hero.onFloor) {
				...
				if (hero.x < floor.x - HERO_WIDTH || hero.x >= floor.x + floor.width-10) {
